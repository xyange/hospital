

package com.wyzy.hospital.pharmacy.controller;

import com.baomidou.mybatisplus.core.toolkit.Wrappers;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.wyzy.hospital.common.core.util.R;
import com.wyzy.hospital.common.log.annotation.SysLog;
import com.wyzy.hospital.pharmacy.entity.Category;
import com.wyzy.hospital.pharmacy.service.CategoryService;
import org.springframework.security.access.prepost.PreAuthorize;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import lombok.AllArgsConstructor;
import org.springframework.web.bind.annotation.*;


/**
 * 药房分类表
 *
 * @author aqie
 * @date 2021-04-22 09:19:47
 */
@RestController
@AllArgsConstructor
@RequestMapping("/category" )
@Api(value = "category", tags = "药房分类表管理")
public class CategoryController {

    private final CategoryService categoryService;

    /**
     * 分页查询
     * @param page 分页对象
     * @param category 药房分类表
     * @return
     */
    @ApiOperation(value = "分页查询", notes = "分页查询")
    @GetMapping("/page" )
    @PreAuthorize("@pms.hasPermission('pharmacy_category_view')" )
    public R getCategoryPage(Page page, Category category) {
        return R.ok(categoryService.page(page, Wrappers.query(category)));
    }


    /**
     * 通过id查询药房分类表
     * @param id id
     * @return R
     */
    @ApiOperation(value = "通过id查询", notes = "通过id查询")
    @GetMapping("/{id}" )
    @PreAuthorize("@pms.hasPermission('pharmacy_category_view')" )
    public R getById(@PathVariable("id" ) Long id) {
        return R.ok(categoryService.getById(id));
    }

    /**
     * 新增药房分类表
     * @param category 药房分类表
     * @return R
     */
    @ApiOperation(value = "新增药房分类表", notes = "新增药房分类表")
    @SysLog("新增药房分类表" )
    @PostMapping
    @PreAuthorize("@pms.hasPermission('pharmacy_category_add')" )
    public R save(@RequestBody Category category) {
        return R.ok(categoryService.save(category));
    }

    /**
     * 修改药房分类表
     * @param category 药房分类表
     * @return R
     */
    @ApiOperation(value = "修改药房分类表", notes = "修改药房分类表")
    @SysLog("修改药房分类表" )
    @PutMapping
    @PreAuthorize("@pms.hasPermission('pharmacy_category_edit')" )
    public R updateById(@RequestBody Category category) {
        return R.ok(categoryService.updateById(category));
    }

    /**
     * 通过id删除药房分类表
     * @param id id
     * @return R
     */
    @ApiOperation(value = "通过id删除药房分类表", notes = "通过id删除药房分类表")
    @SysLog("通过id删除药房分类表" )
    @DeleteMapping("/{id}" )
    @PreAuthorize("@pms.hasPermission('pharmacy_category_del')" )
    public R removeById(@PathVariable Long id) {
        return R.ok(categoryService.removeById(id));
    }

}
